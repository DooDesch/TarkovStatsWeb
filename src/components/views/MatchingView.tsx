...
export function MatchingView({ insights }: MatchingViewProps) {
  ...
  // Simple size guard to avoid Recharts width/height -1 warnings
  ...
  useEffect(() => {
    ...
    if (matchRef.current) {
      const obs = new ResizeObserver((entries) => {
        ...
        setMatchReady(width > 0 && height > 0);
      });
      obs.observe(matchRef.current);
      observers.push(obs);
    }
    ...
    if (startupRef.current) {
      const obs = new ResizeObserver((entries) => {
        const { width, height } = entries[0].contentRect;
        setStartupReady(width > 0 && height > 0);
      });
      obs.observe(startupRef.current);
      observers.push(obs);
    }
    ...
    <div className="space-y-6">
      ...
      {matchData.length > 0 && (
        <ChartContainer title="Matchmaking Duration Over Time" height={280}>
          <div ref={matchRef} className="w-full h-full min-h-[240px]">
            {matchReady && (
      ...
      {startupData.length > 0 && (
        <ChartContainer title="Startup Duration Over Time" height={280}>
          <div ref={startupRef} className="w-full h-full min-h-[240px]">
            {startupReady && (
              <ResponsiveContainer width="100%" height="100%">
